...
## 一、AlibabaProtect 是什么

**AlibabaProtect 是阿里系软件安装时装进系统的常驻后台服务，配有一个自我保护的内核驱动；那些软件卸载后它不随之卸载，而“结束进程、禁用服务、删除文件”这些常规操作都会被回滚或自动恢复——所以必须按第四节的特定顺序清理。**具体身份与机制见下表。
...
以**管理员**身份打开 PowerShell（不会的照做：按 Windows 键输入 `powershell` → 右键搜索结果 → “**以管理员身份运行**” → 弹窗点“是”；窗口标题带“管理员”三个字才算），先看清楚现状：
...
本文档以 **[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)** 发布。本指南为纯文档，不包含任何厂商软件的二进制或本体资源；文中引用的日志/报错原文仅为说明所需的少量引用，全文为作者原创内容。
